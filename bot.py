...
def um(uid):
    # Fixed menu order: built-in buttons first, all custom buttons next,
    # and Support is ALWAYS the last user-facing button.
    k=[
        [kbbtn(text="Account")],
    ...
    custom=c.execute(
        "SELECT name FROM custom_buttons WHERE active=1 ORDER BY id"
    ).fetchall()

    # Add custom buttons in two columns, before Support.
    for i in range(0, len(custom), 2):
        k.append([kbbtn(text=x["name"]) for x in custom[i:i+2]])

    # Support must always remain at the bottom, even after new buttons
    # are added from Admin Panel -> New Button -> Add New Button.
...
async def send_payment_approval_report(bot, w):
    ...
    try:
        total = c.execute(
            ...
            "FROM withdrawals WHERE user_id=? AND status='approved'",
            (w["user_id"],)
        ).fetchone()["total"]

        report_text = (
            "━━━━━━━━━━━━━━━━━━\\n"
            "{{PE:5420592870396557986}} NEW PAYMENT APPROVED\\n"
            "━━━━━━━━━━━━━━━━━━\\n\\n"
            "{{PE:5325971446625758812}} Name : "
            f"{w['full_name'] or 'N/A'}\\n"
            "{{PE:5291913773307667021}} Username     : "
            f"@{w['username'] or 'N/A'}\\n\\n"
            "{{PE:6190336264940559752}} Withdrawal Amount : "
            f"{float(w['amount']):.2f} টাকা\\n"
            "{{PE:5854727876913401191}} Payment Number   : "
            f"{mask_payment_number(w['wallet'])}\\n\\n"
            "━━━━━━━━━━━━━━━━━━\\n"
            "{{PE:5391218287684116768}} Total Withdrawal : "
            f"{float(total):.2f} টাকা\\n"
            "━━━━━━━━━━━━━━━━━━\\n\\n"
            "{{PE:6066904368051916008}} "
            "Payment has been approved successfully."
        )

        await sendmsg(bot, target, report_text)
    except Exception:
        pass
