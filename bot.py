...
def premiumize(text: str):
    """Render {{PE:<id>}} markers as Telegram custom emoji entities."""
    marker = re.compile(r"\{\{PE:(\d+)\}\}")
    chunks = []
    entities = []
    cursor = 0
    utf16_offset = 0

    def u16_len(value: str) -> int:
        return len(value.encode("utf-16-le")) // 2

    for match in marker.finditer(text):
        plain = text[cursor:match.start()]
        chunks.append(plain)
        utf16_offset += u16_len(plain)

        # Fallback character. Telegram requires the custom emoji entity
        # to wrap exactly one valid regular emoji.
        fallback = "⭐"
        ...
        entities.append(
            MessageEntity(
                type="custom_emoji",
                offset=utf16_offset,
                length=u16_len(fallback),
                custom_emoji_id=match.group(1),
            )
        )
        utf16_offset += u16_len(fallback)
        cursor = match.end()

    tail = text[cursor:]
    chunks.append(tail)
    return "".join(chunks), entities
async def recover_submission_file(bot, submission):
    """Return a usable local XLSX path, recovering from Telegram when needed."""
    p = Path(submission.get("file_path", ""))
    if p.exists():
        return p
    ...
    if file_id:
        try:
            ...
            fallback = Path("uploads") / f"{submission.get('id', 'submission')}.xlsx"
            fallback.parent.mkdir(parents=True, exist_ok=True)
            await bot.download_file(tg_file.file_path, destination=str(fallback))
            return fallback
        except Exception:
            pass
    return None
...
async def sendmsg(bot, chat_id, text, reply_markup=None, **kwargs):
    """Send messages with custom emoji entities when PE markers are present."""
    if kwargs.get("entities") is None:
        text, entities = premiumize(text)
        kwargs["entities"] = entities or None
...
def pending_admin_kb():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [_KeyboardButton(text="User Management"), _KeyboardButton(text="Force Join")],
            [_KeyboardButton(text="Work/Price Management")],
            [_KeyboardButton(text="Withdrawal Management"), _KeyboardButton(text="Pending Files")],
            [_KeyboardButton(text="Pending Withdrawals"), _KeyboardButton(text="Generate Main Excel")],
            [_KeyboardButton(text="Upload Report File"), _KeyboardButton(text="Future Reports")],
            [_KeyboardButton(text="Payment Approve Channel")],
            [_KeyboardButton(text="New Button"), _KeyboardButton(text="Set Receiver Channel")],
            [_KeyboardButton(text="Broadcast"), _KeyboardButton(text="Support Management")],
...
async def force_join_callback(q):
    ...
    if kb:
        ...
        await ans(
            q.message,
            "🎉 সবগুলো Channel Join নিশ্চিত হয়েছে!\n\n"
            "এখন আপনি বটের সব ফিচার ব্যবহার করতে পারবেন।",
            reply_markup=um(q.from_user.id)
        )
    ...
    if t=="Pending Files":
        ...
        for s in rows:
            ...
            kb=InlineKeyboardMarkup(inline_keyboard=[[
                ...
                InlineKeyboardButton(text="❌ Reject File",callback_data=f"reject_file:{s['id']}", style="danger")
            ]])
    ...
    try:
        ...
        if a=="reject_reason":
            ...
            premiumize(
                ...
                f"{{{{PE:5420592870396557986}}}} Accepted: {r['accepted_count']}\\n"
                f"{{{{PE:5778318896889074623}}}} Rejected: {r['rejected_count']}\\n"
                f"{{{{PE:6240150562654917631}}}} Added Balance: {r['final_amount']:.2f}"
            ),
...
async def pending_files_direct(m):
    ...
    for s in rows:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            ...
            _InlineKeyboardButton(text="Reject File", callback_data=f"reject_file:{s['id']}", style="danger")
        ]])
...
async def pending_withdrawals_requests_direct(m):
    ...
    for x in rows:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            _InlineKeyboardButton(text="Approve", callback_data=f"wapprove:{x['id']}", style="success"),
            _InlineKeyboardButton(text="Reject + Refund", callback_data=f"wreject:{x['id']}", style="danger")
        ]])
        try:
            await ans(m, text, reply_markup=kb)
            sent += 1
...
async def pending_withdrawals_direct_v13(m):
    ...
    for x in rows:
        ...
        markup = InlineKeyboardMarkup(inline_keyboard=[[
            _InlineKeyboardButton(text="Approve", callback_data=f"wapprove:{wid}", style="success"),
            _InlineKeyboardButton(text="Reject", callback_data=f"wreject:{wid}", style="danger")
        ]])
...
async def pending_withdrawals_menu(m):
    ...
    for x in rows:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="Approve", callback_data=f"wapprove:{x['id']}", style="success"),
            InlineKeyboardButton(text="Reject", callback_data=f"wreject:{x['id']}", style="danger")
        ]])
        try:
            await ans(m, text, reply_markup=kb)
            sent += 1
