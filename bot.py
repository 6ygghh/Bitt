...
async def withdrawal_admin_action(m):
    ...
    if t=="Add Withdrawal Category":
        ...
        return await ans(m, "Category name পাঠান।",reply_markup=am())
    if t=="Delete Category":
        rows=c.execute("SELECT * FROM withdrawal_categories ORDER BY id").fetchall()
        if not rows:
            return await ans(m, "কোনো সেট করা payment category নেই।", reply_markup=am())
        await ans(
            m,
        ...
        return await ans(m, "নিচের বাটন থেকে category নির্বাচন করুন।", reply_markup=am())
    if t in {"Pending Withdrawal Requests","Pending Withdrawals"}:
        ...
        if not rows:
            return await ans(m, "Pending Withdrawal: 0",reply_markup=am())
        for x in rows:
        ...
        return await ans(m, "Pending withdrawal list শেষ।",reply_markup=am())
    ...
    try:
        ...
        if a=="w_add_max":
            ...
            db.commit();state.pop(ADMIN_ID,None)
            return await ans(m, "✅ Withdrawal category added.",reply_markup=am())
        if a=="w_edit_name":
        ...
        if a=="w_edit_max":
            ...
            db.commit();state.pop(ADMIN_ID,None)
            return await ans(m, "✅ Withdrawal category updated.",reply_markup=am())
    except Exception:
        return await ans(m, "Invalid amount/format. আবার চেষ্টা করুন.",reply_markup=am())
