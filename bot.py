...
async def review_confirm_callback(q):
    ...
    try:
        ...
        # Send the normal approval confirmation first, then EDIT that exact
        # user-facing message to append the payment-source footer. This keeps
        # the confirmation as one Telegram message and lets us forward the
        # already-edited message to the configured channel.
        approval_msg = await sendmsg(
            q.message.bot,
            s["user_id"],
            review_complete_text,
        ...
        try:
            bot_info = await q.bot.get_me()
            bot_username = bot_info.username or "YourBotUsername"
        except Exception:
            bot_username = "YourBotUsername"

        payment_source_footer = (
            "\n\n"
            "{{PE:5472250091332993630}} ━━━━━━━━━━━━━━━━━━━\n"
            f"Payment source : @{bot_username}"
        )
        edited_approval_text = review_complete_text + payment_source_footer

        # Edit the confirmation message itself. sendmsg/premiumize() converts
        # the PE marker into Telegram's custom-emoji MessageEntity.
        edited_approval_msg = await q.bot.edit_message_text(
            chat_id=s["user_id"],
            message_id=approval_msg.message_id,
            text=edited_approval_text,
            reply_markup=um(s["user_id"]),
            parse_mode=None,
            entities=premiumize(edited_approval_text)[1]
        )

        # Forward the SAME edited confirmation message to the configured
        # payment-approval channel, so Telegram keeps the custom emoji entity.
        target_channel = setting("payment_approve_channel").strip()
        if target_channel:
            try:
                await q.bot.forward_message(
                    chat_id=target_channel,
                    from_chat_id=s["user_id"],
                    message_id=edited_approval_msg.message_id
                )
            except Exception:
                pass
    except Exception:
