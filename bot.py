...
def premiumize(text: str):
    """Convert {{PE:<custom_emoji_id>}} markers into Telegram custom emoji entities."""
    pattern = re.compile(r"\{\{PE:(\d+)\}\}")
    entities = []
    out = []
    pos = 0

    # Use a regular emoji as the underlying text. Telegram requires a custom
    # emoji entity to wrap exactly one regular emoji.
    fallback = "⭐"

    for match in pattern.finditer(text):
        out.append(text[pos:match.start()])
        offset = len("".join(out).encode("utf-16-le")) // 2
        out.append(fallback)
...
async def sendmsg(bot, chat_id, text, reply_markup=None, **kwargs):
    """Send text while transparently attaching custom-emoji entities."""
