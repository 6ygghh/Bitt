...
import os
import re,io,sqlite3,asyncio,time,hashlib,tempfile,shutil
...
DEFAULT_DISABLE_NOTICE = """╭━━━〔 💎 SUBMISSION NOTICE 〕━━━╮
│
│ 🛑 𝐒𝐔𝐁𝐌𝐈𝐒𝐒𝐈𝐎𝐍 𝐂𝐋𝐎𝐒𝐄𝐃
│
│ 👨‍💼 এডমিন বর্তমানে ফাইল জমা
...
│ 🕖 07:00 PM — 09:30 PM
│
│ ✨ নির্ধারিত সময়ে আবার চেষ্টা করুন।
│
╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯"""

def get_disable_notice():
    return setting("disable_notice", DEFAULT_DISABLE_NOTICE) or DEFAULT_DISABLE_NOTICE
def user(u):
...
def pending_admin_kb():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [_KeyboardButton(text="ডাটাবেজ")],
            [_KeyboardButton(text="মেইনটেন্যান্স")],
...
যখন কোনো Work/Category active থাকবে না, তখন User-কে যে notice দেখাতে চান সেটি এখন পাঠান।

আপনি আপনার premium UI/design সহ পুরো message পাঠাতে পারেন।

বর্তমান notice দেখতে চাইলে /current_notice লিখুন।""",
            reply_markup=ReplyKeyboardMarkup(
                keyboard=[[kbbtn(text="Cancel")]],resize_keyboard=True
            )
        )
...
async def sell(m):
    ...
    k,r=cats()
    if not r:
        return await ans(m, get_disable_notice(), reply_markup=um(m.from_user.id))
...
def maintenance_admin_kb():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [kbbtn(text="মেইনটেন্যান্স ON"), kbbtn(text="মেইনটেন্যান্স OFF")],
            [kbbtn(text="Back to Admin Panel")],
        ],
        resize_keyboard=True,
    )
...
def database_admin_kb():
    return ReplyKeyboardMarkup(
        keyboard=[
            [kbbtn(text="ডাউনলোড ডাটাবেজ")],
            [kbbtn(text="আপলোড ডাটাবেজ")],
            [kbbtn(text="Back to Admin Panel")],
        ],
        resize_keyboard=True,
    )
...
async def admin(m):
    ...
    try:
        ...
        if a=="disable_notice":
            if m.text.strip()=="/current_notice":
                return await ans(m, "বর্তমান Disable Notice:\n\n" + get_disable_notice())
            if m.text.strip()=="Cancel":
                state.pop(ADMIN_ID,None)
                return await ans(m, "Cancelled.", reply_markup=am())
            save_setting("disable_notice", m.text)
            state.pop(ADMIN_ID,None)
            return await ans(m, "✅ Disable Notice সফলভাবে save হয়েছে।\n\nযখন কোনো active Work থাকবে না, User এই notice দেখবে।", reply_markup=am())
        ...
        if a=="work_toggle_id":
            ...
            if not row:
                return await ans(m, "❌ এই Work ID পাওয়া যায়নি। আবার সঠিক ID পাঠান।")
            c.execute("UPDATE categories SET active=? WHERE id=?",(toggle,cid))
            db.commit();state.pop(ADMIN_ID,None)
            if toggle==0:
                notice=(
                    "╭━━━〔 🔔 WORK UPDATE 〕━━━╮\n│\n"
                    f"│ 🛑 <b>{row['name']}</b> কাজটি বর্তমানে বন্ধ করা হয়েছে।\n│\n"
                    "│ 📂 এই Category-তে এখন নতুন ফাইল জমা নেওয়া হচ্ছে না।\n│\n"
                    "│ ✨ অন্য কোনো active Work থাকলে সেটি ব্যবহার করতে পারবেন।\n│\n"
                    "╰━━━━━━━━━━━━━━━━━━━━━━╯"
                )
                sent=failed=0
                users=c.execute("SELECT user_id FROM users WHERE blocked=0").fetchall()
                for u in users:
                    try:
                        await m.bot.send_message(u["user_id"], notice)
                        sent+=1
                    except Exception:
                        failed+=1
                return await ans(m, f"🔕 Work disabled.\n\nCategory: {row['name']}\n📨 Notification sent: {sent}\n⚠️ Failed: {failed}",reply_markup=am())
            return await ans(m, f"🟢 Work '{row['name']}' আবার active করা হয়েছে।",reply_markup=am())
...
dp.message.register(
    ...
    F.from_user.id == ADMIN_ID,
    lambda m: clean_button_text(m.text) in {"মেইনটেন্যান্স", "মেইনটেন্যান্স ON", "মেইনটেন্যান্স OFF", "Back to Admin Panel"}
)
dp.message.register(
    database_admin_action,
    F.from_user.id == ADMIN_ID,
    lambda m: clean_button_text(m.text) in {"ডাটাবেজ", "ডাউনলোড ডাটাবেজ", "আপলোড ডাটাবেজ"} or state.get(ADMIN_ID, {}).get("action") == "database_upload"
)
dp.message.register(
...
dp.message.register(work_menu_action,F.from_user.id==ADMIN_ID, F.text.in_({"Add Work","Edit Work","Disable Work","Enable Work","Disable Notice","Work List","Back to Admin Panel"}))
