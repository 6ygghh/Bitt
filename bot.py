...
async def send_payment_approval_report(bot, w, source_message=None):
    """Edit the exact user confirmation and forward that edited message."""
    target = setting("payment_approve_channel").strip()
    if not target or source_message is None:
        return

    try:
        base_text = source_message.text or ""
        if not base_text:
            return

        # Keep ALL original Telegram entities exactly as Telegram sent them.
        # Do not run premiumize() over the old text, otherwise existing custom
        # emoji entities can be replaced by ordinary ⭐ characters.
        final_text = (
            base_text.rstrip()
            + "\n━━━━━━━━━━━━━━━━━━━\n"
            + "⭐ Payment source : "
            + f"@{(await bot.get_me()).username or 'YourBotUsername'}"
        )

        final_entities = list(source_message.entities or [])
        source_emoji_offset = _u16_len(base_text.rstrip() + "\n━━━━━━━━━━━━━━━━━━━\n")
        final_entities.append(
            MessageEntity(
                type="custom_emoji",
                offset=source_emoji_offset,
                length=1,
                custom_emoji_id="5472250091332993630"
            )
        )

        # EDIT THE SAME USER MESSAGE.
        edited = await bot.edit_message_text(
            ...
            parse_mode=None,
            reply_markup=um(w["user_id"])
        )

        # Forward the EDITED message itself. No new report message is created.
        ...
        print(f"[payment-approve-forward] error: {e}")
...
async def withdrawal_approve_callback(q):
    ...
    try:
        ...
        sent_confirmation = await sendmsg(q.message.bot,w["user_id"],success_text,reply_markup=um(w["user_id"]))
    except Exception:
        pass
    await send_payment_approval_report(q.bot,w,sent_confirmation) if sent_confirmation else None
