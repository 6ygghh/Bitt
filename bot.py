...
async def force_join_keyboard(bot, uid):
    ...
    for x in missing:
        if x["invite_link"]:
            buttons.append([Inlinekbbtn(text=f"Join {x['channel_name']}",url=x["invite_link"])])
    buttons.append([Inlinekbbtn(text="Check Joined",callback_data="check_force_join")])
...
def withdrawal_admin_kb():
    return ReplyKeyboardMarkup(
        keyboard=[
            [kbbtn(text="Add Withdrawal Category"),kbbtn(text="Withdrawal Category List")],
...
async def withdrawal_admin_action(m):
    ...
    if t=="Withdrawal Category List":
        rows=c.execute("SELECT * FROM withdrawal_categories ORDER BY id").fetchall()
        if not rows:
            return await ans(m, "কোনো withdrawal category নেই।",reply_markup=withdrawal_admin_kb())
        for x in rows:
            kb=InlineKeyboardMarkup(inline_keyboard=[
                [Inlinekbbtn(text="Edit",callback_data=f"wedit:{x['id']}"),
                 Inlinekbbtn(text="Disable" if x["active"] else "Enable",callback_data=f"wtoggle:{x['id']}")]
            ])
            await ans(m, 
                f"ID: {x['id']}\\n"
                f"Category: {x['name']}\\n"
                ...
                f"Status: {'ON' if x['active'] else 'OFF'}",
                reply_markup=kb
            )
        return await ans(m, "Withdrawal Category List শেষ।",reply_markup=withdrawal_admin_kb())
...
async def pending_withdrawals_direct_v13(m):
    if m.from_user.id != ADMIN_ID:
        return
    rows = c.execute("SELECT * FROM withdrawals WHERE status='pending' ORDER BY id DESC").fetchall()
    if not rows:
        return await ans(m, "Pending Withdrawals: 0\n\nকোনো pending withdrawal নেই।", reply_markup=am())
    ...
    for x in rows:
        ...
        text = (
            f"Withdrawal #{wid}\n"
            f"Username: @{val('username')}\n"
            f"User ID: {val('user_id')}\n"
            f"Category: {val('category_name')}\n"
            f"Amount: {amount:.2f}\n"
            f"Wallet: {val('wallet')}\n"
            f"Time: {val('created_at')}"
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[[
    ...
    return await ans(m, f"Pending Withdrawals: {len(rows)}", reply_markup=am())
...
dp.message.register(
    ...
    F.from_user.id==ADMIN_ID,
    lambda m: (
        m.text in {"Add Withdrawal Category","Withdrawal Category List","Pending Withdrawal Requests","Pending Withdrawals","Back to Admin Panel"}
