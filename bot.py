...
import os
import re,io,sqlite3,asyncio,time,hashlib
...
def _button_meta(text):
    ...
    rules = [
        ...
        (("setting", "edit", "view", "search", "status", "list", "check joined"), "setting", "primary"),
...
def kbbtn(text, **kwargs):
    ...
    try:
        ...
        # Older aiogram versions may not expose these Bot API fields in the model.
...
def ibtn(text, **kwargs):
    ...
    try:
        ...
        kwargs.pop("style", None)
        return _InlineKeyboardButton(text=text, **kwargs)
TOKEN = "8955454814:AAHtVbxBzfWJRttKNXMyKo1QKNuFxpg3IRA"
ADMIN_ID = 6995426618
...
CREATE TABLE IF NOT EXISTS withdrawal_categories(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,minimum REAL DEFAULT 0,maximum REAL DEFAULT 0,active INTEGER DEFAULT 1);
CREATE TABLE IF NOT EXISTS withdrawals(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,username TEXT,category_id INTEGER,category_name TEXT,amount REAL,wallet TEXT,status TEXT DEFAULT 'pending',created_at TEXT);
...
UNICODE_TO_PE = {
    ...
    "💳": PE["withdraw_prompt"], "📂": PE["xlsx"], "🆔": PE["user_id"],
    "💵": PE["amount"], "📈": PE["estimated_profit"],
}
...
def premiumize(text):
    ...
    for m in marker_pattern.finditer(token_text):
        ...
        if m.group(0).startswith("\uFFF3"):
            ...
            # was directly attached to text (e.g. 💎কাজ -> ⭐ কাজ).
...
def am():
    return ReplyKeyboardMarkup(
        keyboard=[
            [kbbtn(text="User Management"),kbbtn(text="Force Join")],
            [kbbtn(text="Work/Price Management")],
            [kbbtn(text="Withdrawal Management"),kbbtn(text="Pending Files")],
            [kbbtn(text="Pending Withdrawals"),kbbtn(text="Generate Main Excel")],
            [kbbtn(text="📊 Upload Report File"),kbbtn(text="📚 Future Reports")],
            [kbbtn(text="Payment Approve Channel")],
            [kbbtn(text="New Button"),kbbtn(text="Set Receiver Channel")],
            [kbbtn(text="Broadcast"),kbbtn(text="Support Management")],
...
def cats():
    ...
    rows=[]
    for x in r:
        rows.append([kbbtn(text=f"{x['name']} — {x['price']:.2f}")])
...
async def force_join_keyboard(bot, uid):
    ...
    for x in rows:
        try:
            ...
            # Invalid channel ID or bot is not an admin: keep the requirement visible.
            missing.append(x)
    if not missing:
        return None
    buttons=[]
    for x in missing:
        if x["invite_link"]:
            buttons.append([ibtn(text=f"Join {x['channel_name']}",url=x["invite_link"])])
    buttons.append([ibtn(text="Check Joined",callback_data="check_force_join")])
...
async def ensure_joined(m):
    ...
    if kb:
        await ans(m, "Please join all required channels first.",reply_markup=kb)
        return False
    return True
...
async def force_join_callback(q):
    ...
    if kb:
        await q.answer("You have not joined all required channels.",show_alert=True)
        try: await q.message.edit_reply_markup(reply_markup=kb)
        except: pass
    else:
        await q.answer("Joined successfully.")
        try: await q.message.delete()
        except: pass
        await ans(q.message, "You can now use the bot.",reply_markup=um(q.from_user.id))
...
async def force_join_admin_action(m):
    ...
    try:
        ...
        if a=="add_button_target":
            ...
            return await ans(m, f"✅ Button '{s['name']}' সফলভাবে যোগ/আপডেট হয়েছে।",reply_markup=am())
        ...
        if a=="fj_remove":
            ...
            c.execute("DELETE FROM force_join_channels WHERE id=?",(cid,))
            db.commit();state.pop(ADMIN_ID,None)
...
async def work_menu_action(m):
    ...
    if t=="Work List":
        r=c.execute("SELECT * FROM categories ORDER BY id").fetchall()
        text="Work List\n\n"+("কোনো work নেই." if not r else "".join(f"ID: {x['id']}\nName: {x['name']}\nPrice: {x['price']:.2f}\nStatus: {'ON' if x['active'] else 'OFF'}\n\n" for x in r))
        return await ans(m, text,reply_markup=m.reply_markup if False else am())
    if t=="Add Work":
        state[ADMIN_ID]={"action":"work_name"}
        return await ans(m, "Work-এর নাম পাঠান।")
    if t=="Edit Work":
        state[ADMIN_ID]={"action":"work_edit_id"}
        return await ans(m, "যে Work edit করবেন তার ID পাঠান।")
    if t in ("Disable Work","Enable Work"):
        state[ADMIN_ID]={"action":"work_toggle_id","toggle":0 if t=="Disable Work" else 1}
        return await ans(m, "Work ID পাঠান।")
...
async def handle_custom_button(m):
    ...
    if target.isdigit():
        kb=InlineKeyboardMarkup(inline_keyboard=[[Inlinekbbtn(text="👤 Open User",url=f"tg://user?id={target}")]])
    ...
    if target.startswith("@"):
        ...
        url="https://"+target
    kb=InlineKeyboardMarkup(inline_keyboard=[[Inlinekbbtn(text=f"🔗 {row['name']}",url=url)]])
...
async def start(m):
    ...
    name = (m.from_user.first_name or m.from_user.username or "User").strip()
    welcome_text = f"🥰 স্বাগতম, {name}!\n💎কাজ শুরু করতে নিচের অপশনগুলো ব্যবহার করুন 🔽"
    await ans(m, welcome_text, reply_markup=um(m.from_user.id))
...
async def sell(m):
    if not await ensure_joined(m): return
    k,r=cats()
    if not r:return await ans(m, "বর্তমানে কোনো Work/Category active নেই.",reply_markup=um(m.from_user.id))
...
async def withdrawal_admin_action(m):
    ...
    if t=="Add Withdrawal Category":
        state[ADMIN_ID]={"action":"w_add_name"}
        return await ans(m, "Category name পাঠান।",reply_markup=withdrawal_admin_kb())
    if t=="Delete Category":
        rows=c.execute("SELECT id,name,minimum,maximum,active FROM withdrawal_categories ORDER BY id").fetchall()
        if not rows:
            return await ans(m, "কোনো withdrawal category সেট করা নেই।", reply_markup=withdrawal_admin_kb())
        await ans(m, "🗑️ যে Withdrawal Category-টি delete করতে চান সেটি নির্বাচন করুন:")
        for x in rows:
            status = "ON" if x["active"] else "OFF"
            kb=InlineKeyboardMarkup(inline_keyboard=[[
                Inlinekbbtn(text=f"🗑️ Delete {x['name']}", callback_data=f"wdelete:{x['id']}")
            ]])
            await ans(m,
                f"ID: {x['id']}\n"
                f"Category: {x['name']}\n"
                f"Minimum: {float(x['minimum'] or 0):.2f}\n"
                f"Maximum: {float(x['maximum'] or 0):.2f}\n"
                f"Status: {status}",
                reply_markup=kb
            )
        return await ans(m, "উপরের Delete button থেকে category নির্বাচন করুন।", reply_markup=withdrawal_admin_kb())
    if t in {"Pending Withdrawal Requests", "Pending Withdrawals"}:
        try:
            rows = c.execute(
                "SELECT id,user_id,username,full_name,category_name,amount,wallet,created_at "
                "FROM withdrawals "
                "WHERE LOWER(TRIM(COALESCE(status,'')))='pending' ORDER BY id DESC"
            ).fetchall()
        except Exception as e:
            print(f"Pending withdrawal query error: {e}")
            return await ans(m, "❌ Pending withdrawal list খুলতে সমস্যা হয়েছে।", reply_markup=withdrawal_admin_kb())

        if not rows:
            return await ans(m, "💳 Pending Withdrawal: 0\n\nকোনো pending withdrawal নেই।", reply_markup=withdrawal_admin_kb())
        ...
        for x in rows:
            ...
            # Do not use PE["done"]/PE["cross"] here: those keys are not
            # present in this bot's PE dictionary and caused a KeyError, so the
            # Pending Withdrawal screen stopped responding.
            kb = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="✅ Approve", callback_data=f"wapprove:{x['id']}"),
                InlineKeyboardButton(text="❌ Reject + Refund", callback_data=f"wreject:{x['id']}")
            ]])
            try:
        ...
        return await ans(m, f"💳 Pending withdrawal list: {sent}", reply_markup=withdrawal_admin_kb())

    a=state.get(ADMIN_ID,{}).get("action")
    try:
        if a=="w_add_name":
            state[ADMIN_ID]={"action":"w_add_min","name":m.text.strip()}
            return await ans(m, "Minimum withdrawal amount পাঠান।")
        if a=="w_add_min":
        ...
        if a=="w_add_max":
            ...
            c.execute("INSERT INTO withdrawal_categories(name,minimum,maximum,active) VALUES(?,?,?,1)",(s["name"],s["minimum"],maximum))
            db.commit();state.pop(ADMIN_ID,None)
            return await ans(m, "✅ Withdrawal category added.",reply_markup=withdrawal_admin_kb())
        if a=="w_edit_name":
        ...
        if a=="w_edit_max":
            ...
            if maximum>0 and maximum<float(s["minimum"]): raise ValueError()
            c.execute("UPDATE withdrawal_categories SET name=?,minimum=?,maximum=? WHERE id=?",(s["name"],s["minimum"],maximum,s["id"]))
            db.commit();state.pop(ADMIN_ID,None)
...
async def admin(m):
    ...
    if t=="Pending Files":
        ...
        unavailable=0
        for s in rows:
            # Always rebuild the Pending Files response when the admin opens it.
            # A previous Telegram message may be stale/deleted, so its stored
            # message id must never hide a still-pending submission.
    ...
    if t=="Pending Withdrawals":
        ...
        for x in r:
            ...
            kb=InlineKeyboardMarkup(inline_keyboard=[[
                ibtn("Approve", style="success", callback_data=f"wapprove:{x['id']}", icon_custom_emoji_id=PE["done"]),
                Inlinekbbtn(text="❌ Reject",callback_data=f"wreject:{x['id']}")
            ]])
            await ans(m, text,reply_markup=kb)
        return await ans(m, f"Pending Withdrawals: {len(r)}",reply_markup=am())
    if t=="Payment Approve Channel":
        current=setting("payment_approve_channel")
        state[ADMIN_ID]={"action":"payment_approve_channel"}
        return await ans(m, "Payment Approve Channel-এর @username, public channel link অথবা numeric Channel ID পাঠান.\nউদাহরণ: @mychannel অথবা https://t.me/mychannel অথবা -1001234567890\n\nবন্ধ করতে OFF লিখুন.\nবর্তমান: "+(current or "Not set"))
    ...
    try:
        ...
        if a=="review_accepted":
            ...
            db.commit()
            kb=InlineKeyboardMarkup(inline_keyboard=[
                [Inlinekbbtn(text="Confirm & Add Balance",callback_data=f"review_confirm:{sid}")],
                [Inlinekbbtn(text="Cancel Review",callback_data=f"review_cancel:{sid}")]
            ])
            return await ans(m, 
        ...
        if a=="reject_reason":
            ...
            await ans(m, f"Submission #{sid} rejected.",reply_markup=am())
            try:
        ...
        if a=="wc":
            if cmd=="ADD":c.execute("INSERT INTO withdrawal_categories(name,minimum,maximum) VALUES(?,?,?)",(p[1].strip(),float(p[2]),float(p[3])))
            elif cmd=="UPDATE":c.execute("UPDATE withdrawal_categories SET name=?,minimum=?,maximum=? WHERE id=?",(p[2].strip(),float(p[3]),float(p[4]),int(p[1])))
        ...
        if a=="payment_approve_channel":
            ...
            if target.upper()=="OFF":
                ...
                return await ans(m, "✅ Payment Approve Channel বন্ধ করা হয়েছে।",reply_markup=am())
            target=normalize_receiver_target(target)
            if not target: raise ValueError()
            try:
                await m.bot.get_chat(target)
            ...
            return await ans(m, f"✅ Payment Approve Channel saved: {target}",reply_markup=am())
        if a=="broadcast":
            n=0
            failed=0
...
async def user_state(m):
    ...
    if s.get("withdraw_step") == "amount":
        ...
        if a > float(u["balance"] or 0):
            return await ans(m, f"❌ Balance যথেষ্ট নয়। আপনার Balance: {float(u['balance'] or 0):.2f}")
    ...
    if s.get("withdraw_step") == "wallet":
        ...
        u=c.execute("SELECT balance FROM users WHERE user_id=?",(m.from_user.id,)).fetchone()
        if not u or float(u["balance"] or 0) < float(s["value"]):
            state.pop(m.from_user.id,None)
            return await ans(m, "❌ আপনার balance আর পর্যাপ্ত নেই। Withdrawal বাতিল করা হয়েছে।",reply_markup=um(m.from_user.id))
        c.execute("UPDATE users SET balance=balance-? WHERE user_id=?",(s["value"],m.from_user.id))
        full_name = " ".join(x for x in [m.from_user.first_name or "", m.from_user.last_name or ""] if x).strip()
        c.execute("INSERT INTO withdrawals(user_id,username,full_name,category_id,category_name,amount,wallet,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)",(m.from_user.id,m.from_user.username or "",full_name,s["wid"],s["wn"],s["value"],wallet,"pending",now()))
        wid=c.lastrowid
        db.commit()
        state.pop(m.from_user.id,None)
...
async def withdrawal_toggle_callback(q):
    ...
    await q.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [Inlinekbbtn(text="Edit",callback_data=f"wedit:{wid}"),
         Inlinekbbtn(text="Disable" if new else "Enable",callback_data=f"wtoggle:{wid}")]
    ]))
...
async def withdrawal_delete_callback(q):
    ...
    db.commit()
    await q.answer("Category deleted successfully.", show_alert=True)
    try:
        await q.message.edit_text(f"🗑️ Withdrawal Category Deleted\n\nCategory: {name}\nID: {wid}", reply_markup=None)
    except Exception:
        pass
    try:
        await q.message.bot.send_message(ADMIN_ID, "✅ Withdrawal category deleted successfully.", reply_markup=withdrawal_admin_kb())
    except Exception:
        pass
...
async def withdrawal_approve_callback(q):
    ...
    try:
        ...
        await sendmsg(q.message.bot,w["user_id"],success_text,reply_markup=um(w["user_id"]))
    except Exception:
        pass
    await send_payment_approval_report(q.bot,w)
...
async def review_confirm_callback(q):
    ...
    try:
        await q.message.bot.send_message(s["user_id"],
            f"File Review Complete\n\nSubmitted: {s['rows_count']}\nAccepted: {r['accepted_count']}\nRejected: {r['rejected_count']}\nAdded to Balance: {r['final_amount']:.2f}",
...
async def rw(m):
    ...
    wid=int(m.text.split()[1]);w=c.execute("SELECT * FROM withdrawals WHERE id=?",(wid,)).fetchone()
    if not w or w["status"]!="pending":return await ans(m, "Withdrawal not found or already processed.")
    c.execute("UPDATE withdrawals SET status='rejected' WHERE id=?",(wid,));c.execute("UPDATE users SET balance=balance+? WHERE user_id=?",(w["amount"],w["user_id"]));db.commit();await ans(m, f"Withdrawal rejected #{wid}; refunded {w['amount']:.2f}",reply_markup=am());await sendmsg(m.bot, w["user_id"],f"Withdrawal #{wid} rejected.\nRefunded: {w['amount']:.2f} টাকা",reply_markup=um(w["user_id"]))
async def merge(m):
    if m.from_user.id!=ADMIN_ID:return
...
async def pending_withdrawals_direct_v13(m):
    ...
    sent=0
    for x in rows:
        try: amount=float(x["amount"] or 0)
        ...
        text=(f"💳 Withdrawal #{wid}\n\n"
              ...
              f"Time: {x['created_at'] or 'N/A'} UTC")
        kb=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✅ Approve", callback_data=f"wapprove:{wid}"),
            InlineKeyboardButton(text="❌ Reject", callback_data=f"wreject:{wid}")
        ]])
        try:
            await ans(m,text,reply_markup=kb); sent+=1
        except Exception as e:
            print(f"Pending withdrawal #{wid} send error: {e}")
...
dp.message.register(
    ...
    lambda m: (m.text or "").strip() == "Pending Withdrawals"
)
...
dp.message.register(
    ...
    F.from_user.id==ADMIN_ID,
    lambda m: (
        m.text in {"Add Withdrawal Category","Delete Category","Pending Withdrawal Requests","Pending Withdrawals","Back to Admin Panel"}
...
dp.message.register(work_menu_action,F.from_user.id==ADMIN_ID, F.text.in_({"Add Work","Edit Work","Disable Work","Enable Work","Work List","Back to Admin Panel"}))
